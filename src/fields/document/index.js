...
export default class DocumentField extends Component {
   ...
   */
  renderFileItem = ({ item }) => {
    const { theme, handlePreviewDocument, AppNBText } = this.props;
    return (
      <View
  ...
  renderAllDocuments = (attributes) => {
    ...
    if (
      !isEmpty(value) &&
      (_.some(value, "uri") || _.some(value, "file_path"))
    ) {
      _.forEach(value, (file) => {
        data.push(file);
      });
    }
    return <View style={styles.topContainer}>{this.renderFileList(data)}</View>;
  };
  ...
  uploadSingleFile = (config, res) => {
    ...
    } else if (!config["multiple"] && !isEmpty(value) && value.length >= 1) {
  ...
  uploadMultipleFiles = (config, results) => {
    ...
    if (!error && updatedResults.length > 0) {
      if (typeof handleDocumentUpdateAndDownload === "function") {
        handleDocumentUpdateAndDownload(attributes, updatedResults);
      }
    }
  };
