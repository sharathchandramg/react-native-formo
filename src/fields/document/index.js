...
export default class DocumentField extends Component {
  ...
  renderPreview = attributes => {
    ...
    console.log(data);
    return (
      <View style={[styles.topContainer]}>{this.renderFileList(data)}</View>
    );
  };
  ...
  getDocumentConfiguration = () => {
    ...
    if (!isEmpty(additional_config)) {
      if (!isEmpty(additional_config['file_type'])) {
        ['pdf', 'doc'].forEach(item => {
          documentTypes = [...documentTypes, ...allowedDocumentTypes[item]];
  ...
  singleFileValidationsAndUpload = (config, res) => {
    ...
    } else if (!config['multiple'] && value.length >= 1) {
  ...
  multiFileValidationsAndUpload = (config, results) => {
    ...
    if (config['multiple'] && allFiles.length > config['maxFiles']) {
      error = true;
      this.renderAlert(`Maximum files to be allowed is ${config['maxFiles']}`);
      return;
    }
    ...
    for (const res of results) {
      if (!config['contentTypes'].includes(res['type'])) {
        ...
        error = true;
        this.renderAlert(
          `${res['name']} file size is empty. Please choose proper file`
        );
        return;
      } else if (res['size'] > config['maxSize']) {
        error = true;
        this.renderAlert(
          `${res['name']} file size is greater than ${this.getBytesToMB(
            config['maxSize']
    ...
    const updatedResults = !isEmpty(existsFiles)
      ? this.removeDuplicateFiels(results, existsFiles)
      : results;

    if (!error && updatedResults.length > 0) {
      if (typeof handleDocumentUpdateAndDownload === 'function') {
        handleDocumentUpdateAndDownload(attributes, updatedResults, 'write');
      }
    }
  };
    ...
    return (
      <View>
        <View>
          ...
          {this.isDocumentFilesExists() ? this.renderPreview(attributes) : null}
        </View>
