...
export default class LookupField extends Component {
  ...
  handleOnSearchQuery = (searchText, lookAhead) => {
    ...
    if (searchText) {
      ...
      this.setState({
        searchModalVisible:
          typeof lookAhead !== "undefined" && lookAhead ? true : false,
    ...
    if (categoryToValue.length && index !== -1) {
      ...
      if (index !== -1) {
        ...
        if (options.includes(item["value"])) {
          options = _.filter(options, (option) => option !== item["value"]);
        } else {
  ...
  isBarcodeEnable = (attributes) => {
    if (
      !isEmpty(attributes) &&
      typeof attributes["barcodeKey"] !== "undefined" &&
      attributes["barcodeKey"]
    ) {
      return true;
    }
    return false;
    ...
    return (
      <View style={styles.container}>
        ...
        {this.state.modalVisible && (
          ...
          >
            {this.state.searchModalVisible || this.state.filterModalVisible ? (
